...
# interface power
# 175mW for the DIMM, assuming 8 chips/DIMM as below
DRAM_POWER_STATIC_OFFCHIP_INTERFACE = .175/8
...
def compute_dram_power(nread, nwrite, t, config):
    ...
    if num_dram_controllers > 0:
        ...
        sockets = math.ceil(float(config['general/total_cores']) /
                            float(config['perf_model/dram/controllers_interleaving']))
    chips_per_dimm = float(config.get(
        'perf_model/dram/chips_per_dimm', DRAM_CHIPS_PER_DIMM))
    dimms_per_socket = float(config.get(
        'perf_model/dram/dimms_per_controller', DRAM_DIMMS_PER_SOCKET))
...
def log_frequencies(results):
    ...
    frequencies = [float(
        results['config']['perf_model/core/frequency'].get(i)) for i in range(ncores)]
...
def log_vdd(results):
    ...
    vdd = [float(results['config']['power/vdd'].get(i))
           * scale for i in range(ncores)]
...
def log_cpi_stack(results):
    ...
    for filename, write_header, mode in ((cpi_stack_file, True, 'w'),
        ...
        with open(filename, mode) as f:
            ...
            f.write('total\t')
            f.write('\t'.join('{:.3f}'.format(
                sum(cpiStackData[i].values())) for i in range(ncores)))
            f.write('\n')
            ...
            for label in labels:
                if any(label.startswith(c) for c in compactify):
                    ...
                    values = [cpiStackData[i].get(label, 0)
                              for i in range(ncores)]
...
def main(jobid, resultsdir, outputfile, powertype='dynamic', config=None, no_graph=False, partial=None, print_stack=True, return_data=False):
    ...
    if config:
        ...
        results['config'] = sniper_config.parse_config(
            file(config).read(), results['config'])

        # recompute cycle counts with updated frequencies
        _results = sniper_lib.parse_results_from_dir(
            resultsdir, partial=partial, metrics=None)
        results['results'] = sniper_lib.stats_process(
            results['config'], _results)

    stats = sniper_stats.SniperStats(resultsdir=resultsdir, jobid=jobid)

    power, nuca_at_level = edit_XML(
        stats, results['results'], results['config'])
    ...
    if powertype == 'area':
        ...
        for core, (res, total, other, scale) in results.items():
            ...
            for name, value in res:
                if print_stack:
                    energy, energy_scale = sniper_lib.scale_sci(
                        float(value) * seconds)
    ...
    if not no_graph:
        ...
        plot_labels_with_color = [
            n for n in all_names_with_colors if n[0] in plot_labels]
        gnuplot.make_stacked_bargraph(
            outputfile, plot_labels_with_color, plot_data, 'Energy (J)')
...
def power_stack(power_dat, cfg, seconds, powertype='total', nocollapse=False):
    ...
    def getpower(powers, key=None):
        def getcomponent(suffix):
            if key:
                return scale_power(suffix, powers.get(key+'/'+suffix, 0), size_nm)
            else:
                return scale_power(suffix, powers.get(suffix, 0), size_nm)
    ...
    data = {
        ...
        + sum([getpower(core, 'L2')
               for core in power_dat['Core']]),  # private L2
    ...
    data['core-other'] = getpower(power_dat['Processor']) - \
        (sum(data.values()) - data['dram'])
    ...
    if (sniper_config.get_config(cfg, "periodic_thermal/enabled") == 'true'):
        ...
        if not needInitializing:
            hotspot_args += ['-init_file', os.path.join(
                sniper_config.get_config(cfg, "general/output_dir"), 'Temperature.init')]
...
def edit_XML(statsobj, stats, cfg):
    ...
    l2_cacheSharedCores = long(sniper_config.get_config_default(
        cfg, 'perf_model/l2_cache/shared_cores', 0))
    nuca_at_level = False
    private_l2s = True
    ...
    if long(sniper_config.get_config_default(cfg, 'perf_model/l2_cache/data_access_time', 0)) > 0:
        ...
        private_l2s = int(sniper_config.get_config(
    ...
    if int(cfg['perf_model/cache/levels']) >= 3:
        ...
        if cfg['perf_model/dram_directory/locations'] == 'interleaved':
            nuca_cacheSharedCores = int(
                cfg['perf_model/dram_directory/interleaving'])
            num_nucas = int(math.ceil(ncores / float(nuca_cacheSharedCores)))
        else:
            nuca_locations = [
                lid for name, lid, mid in statsobj.get_topology() if name == 'nuca-cache']
            ...
            nuca_locations_assumed = [
                i*nuca_cacheSharedCores for i in range(num_nucas)]
            if nuca_locations != nuca_locations_assumed:
                raise ValueError('Unsupported tag directory locations %s' %
                                 cfg['perf_model/dram_directory/locations'])
        if num_l2s == 0:
        ...
        cfg['perf_model/l%d_cache/data_access_time' %
            l] = cfg['perf_model/nuca/data_access_time']
        cfg['perf_model/l%d_cache/associativity' %
            l] = cfg['perf_model/nuca/associativity']
        cfg['perf_model/l%d_cache/cache_block_size' %
            l] = cfg['perf_model/l2_cache/cache_block_size']
        cfg['perf_model/l%d_cache/cache_size' %
            l] = cfg['perf_model/nuca/cache_size']
...
# ---------------------------
    ...
    for core in range(ncores):
        data[core]['idle_cycles'] = cycles_scale[core] * \
            stats['performance_model.idle_elapsed_time'][core]
        ...
        data[core]['Branch_instructions'] = (stats.get(
            'interval_timer.uop_branch', stats.get('rob_timer.uop_branch', []))[core])
        data[core]['ialu_accesses'] = (stats.get('interval_timer.uop_load', stats.get('rob_timer.uop_load', []))[core]) \
            ...
            + (stats.get('interval_timer.uop_generic',
                         stats.get('rob_timer.uop_generic', []))[core])
    ...
    for i in xrange(len(template)-1):
        ...
        def get_clock(component):
            domain = sniper_config.get_config(
                cfg, component+'/dvfs_domain', core)
            if domain == 'core':
                return clock_core
            elif domain == 'global':
                return clock_global
        ...
        def get_vdd(component):
            ...
            if domain == 'core':
                ...
                raise ValueError('Unknown DVFS domain %s' % domain)
        issue_width = long(sniper_config.get_config(
            cfg, 'perf_model/core/interval_timer/dispatch_width', core))
        peak_issue_width = long(long(sniper_config.get_config(
            cfg, 'perf_model/core/interval_timer/dispatch_width', core)) * 1.5)
        ALU_per_core = peak_issue_width
        window_size = int(sniper_config.get_config(
            cfg, "perf_model/core/interval_timer/window_size", core))
        if sniper_config.get_config(cfg, "perf_model/core/type", core) == 'rob' and sniper_config.get_config_bool(cfg, "perf_model/core/rob_timer/in_order", core):
            ...
            machineType = 0  # OoO
        latency_bp = long(sniper_config.get_config(
            cfg, 'perf_model/branch_predictor/mispredict_penalty', core))
        # ---FROM CONFIG----------
        latency_l1_d = long(sniper_config.get_config(
            cfg, 'perf_model/l1_dcache/data_access_time', core))
        latency_l1_i = long(sniper_config.get_config(
            cfg, 'perf_model/l1_icache/data_access_time', core))
        latency_l2 = long(sniper_config.get_config_default(
            cfg, 'perf_model/l2_cache/data_access_time', 0, core))
        latency_l3 = long(sniper_config.get_config_default(
            cfg, 'perf_model/l3_cache/data_access_time', 0, core))
        l1_dcacheAssociativity = power2up(long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_dcache/associativity', 0, core)))
        l1_dcacheBlockSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_dcache/cache_block_size', 0, core))
        l1_dcacheSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_dcache/cache_size', 0, core))
        l1_dcacheSharedCores = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_dcache/shared_cores', 0, core))
        l1_dcacheWriteBackTime = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_dcache/writeback_time', 0, core))
        l1_icacheAssociativity = power2up(long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_icache/associativity', 0, core)))
        l1_icacheBlockSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_icache/cache_block_size', 0, core))
        l1_icacheSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_icache/cache_size', 0, core))
        l1_icacheSharedCores = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_icache/shared_cores', 0, core))
        l1_icacheWriteBackTime = long(sniper_config.get_config_default(
            cfg, 'perf_model/l1_icache/writeback_time', 0, core))
        l2_cacheAssociativity = power2up(long(sniper_config.get_config_default(
            cfg, 'perf_model/l2_cache/associativity', 0, core)))
        l2_cacheSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l2_cache/cache_size', 0, core))
        l2_cacheBlockSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l2_cache/cache_block_size', 0, core))
        l2_cacheWriteBackTime = long(sniper_config.get_config_default(
            cfg, 'perf_model/l2_cache/writeback_time', 0, core))
        l3_cacheAssociativity = power2up(long(sniper_config.get_config_default(
            cfg, 'perf_model/l3_cache/associativity', 0, core)))
        l3_cacheBlockSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l3_cache/cache_block_size', 0, core))
        l3_cacheSize = long(sniper_config.get_config_default(
            cfg, 'perf_model/l3_cache/cache_size', 0, core))
        l3_cacheWriteBackTime = long(sniper_config.get_config_default(
            cfg, 'perf_model/l3_cache/writeback_time', 0, core))
        ...
        if template[i][1]:
            ...
            if len(template[i][1]) == 1:
                ...
                if template[i][1][0] == "core_clock":
                    ...
                    template[i][0] = template[i][0] % get_clock(
                        'perf_model/l2_cache')
                    ...
                    template[i][0] = template[i][0] % get_vdd(
                        'perf_model/l2_cache')
                    ...
                    raise ValueError('Unknown cfg template %s' %
                                     template[i][1][0])
            elif template[i][1][1] == "stat":
                cores_l2s = range(l2_cacheSharedCores*core, min(ncores,
                                                                l2_cacheSharedCores*core+l2_cacheSharedCores))
                cores_l3s = range(l3_cacheSharedCores*core, min(ncores,
                                                                l3_cacheSharedCores*core+l3_cacheSharedCores))
                # core statistics
                if template[i][1][0] == "total_cycles":
                    ...
                    template[i][0] = template[i][0] % (
                        cycles[core] - data[core]['idle_cycles'])
                    ...
                    if float(((instrs[core]))/max_system_cycles) > 1:
                        ...
                        template[i][0] = template[i][0] % float(
                            ((instrs[core]))/max_system_cycles)
                elif template[i][1][0] == "LSU.duty_cycle":
                    if float((long(stats['L1-D.loads'][core])+long(stats['L1-D.stores'][core]))/max_system_cycles) <= 1:
                        template[i][0] = template[i][0] % float(
                            (long(stats['L1-D.loads'][core])+long(stats['L1-D.stores'][core]))/max_system_cycles)
                    else:
                        template[i][0] = template[i][0] % 1
                    ...
                    if float((long(stats['L1-I.loads'][core])+long(stats['L1-I.stores'][core]))/max_system_cycles) <= 1:
                        template[i][0] = template[i][0] % float(
                            (long(stats['L1-I.loads'][core])+long(stats['L1-I.stores'][core]))/max_system_cycles)
                    else:
                        template[i][0] = template[i][0] % 1
                    ...
                    template[i][0] = template[i][0] % min(
                        1, float((int(data[core]['FP_instructions']))/max_system_cycles))
                elif template[i][1][0] == "MUL.duty_cycle":
                    template[i][0] = template[i][0] % min(1, float((stats.get(
                        'interval_timer.uop_fp_muldiv', stats.get('rob_timer.uop_fp_muldiv', []))[core])/max_system_cycles))
                # check whether it is per FP  basis or total
                elif template[i][1][0] == "ALU.duty_cycle":
                    template[i][0] = template[i][0] % min(
                        1, ((instrs[core] - data[core]['FP_instructions'])/(max_system_cycles * ALU_per_core)))
                    ...
                    template[i][0] = template[i][0] % (
                        int(DRAM_reads) + int(DRAM_writes))
                    ...
                    if 'network.shmem-1.mesh.link-in.num-requests' in stats:
                        template[i][0] = template[i][0] % sum(
                            stats['network.shmem-1.mesh.link-in.num-requests'])
                    elif 'network.shmem-1.mesh.packets-in' in stats:
                        template[i][0] = template[i][0] % sum(
                            stats['network.shmem-1.mesh.packets-in'])
                    elif 'network.shmem-1.bus.num-requests' in stats:
                        template[i][0] = template[i][0] % int(
                            stats['network.shmem-1.bus.num-requests'][0])  # assumption
                    elif 'network.shmem-1.bus.num-packets' in stats:
                        template[i][0] = template[i][0] % int(
                            stats['network.shmem-1.bus.num-packets'][0])  # assumption
                    else:
                        template[i][0] = template[i][0] % int(
                            stats['bus.num-requests'][0])  # assumption
                elif template[i][1][0] == "NoC.duty_cycle":
                    if 'network.shmem-1.mesh.link-left.total-time-used' in stats:
                        DIRECTIONS = ('up', 'down', 'left', 'right')
                        total_time_used = sum(
                            [sum(stats['network.shmem-1.mesh.link-%s.total-time-used' % direction]) for direction in DIRECTIONS])
                        num_links_used = sum(
                            [sum([v > 0 and 1 or 0 for v in stats['network.shmem-1.mesh.link-%s.num-requests' % direction]]) for direction in DIRECTIONS])
                        ...
                        avg_time_used = total_time_used / \
                            float(num_links_used or 1.)
                        duty_cycle = avg_time_used / \
                            (stats['global.time'] or 1.)
                        ...
                        template[i][0] = template[i][0] % min(
                            1, cycles_scale[core]*float(stats['network.shmem-1.bus.time-used'][0])/max_system_cycles)
                    else:
                        template[i][0] = template[i][0] % min(
                            1, cycles_scale[core]*float(stats['bus.time-used'][0])/max_system_cycles)
                elif template[i][1][0] == "loads":
                    template[i][0] = template[i][0] % long(
                        stats['L1-D.loads'][core])
                elif template[i][1][0] == "stores":
                    template[i][0] = template[i][0] % long(
                        stats['L1-D.stores'][core])
                elif template[i][1][0] == "total_instructions":
                    template[i][0] = template[i][0] % instrs[core]
                elif template[i][1][0] == "integer_ins":
                    template[i][0] = template[i][0] % int(int(
                        instrs[core]) - int(data[core]['FP_instructions']) - int(data[core]['Branch_instructions']))
                elif template[i][1][0] == "fp_ins":
                    template[i][0] = template[i][0] % int(
                        data[core]['FP_instructions'])
                elif template[i][1][0] == "itlb_total_accesses":
                    template[i][0] = template[i][0] % int(instrs[core]*0.5)
                elif template[i][1][0] == "itlb_misses":
                    template[i][0] = template[i][0] % int(
                        instrs[core]*(0.5*0.5/10000))
                elif template[i][1][0] == "BTB.read_accesses":
                    template[i][0] = template[i][0] % int(
                        data[core]['Branch_instructions'])  # instrs[core]
                    ...
                    template[i][0] = template[i][0] % int(
                        2 * int(data[core]['FP_instructions']))
                    ...
                    template[i][0] = template[i][0] % int(
                        data[core]['ialu_accesses'])
                elif template[i][1][0] == "window_switches.fpu_accesses":
                    template[i][0] = template[i][0] % int(
                        data[core]['FP_instructions'])
                elif template[i][1][0] == "window_switches.mul_accesses":
                    template[i][0] = template[i][0] % int(stats.get(
                        'interval_timer.uop_fp_muldiv', stats.get('rob_timer.uop_fp_muldiv', []))[core])
                    ...
                    template[i][0] = template[i][0] % int(stats.get(
                        'interval_timer.uops_total', stats.get('rob_timer.uops_total', []))[core]*1)
                elif template[i][1][0] == "ROB_writes":
                    template[i][0] = template[i][0] % int(stats.get(
                        'interval_timer.uops_total', stats.get('rob_timer.uops_total', []))[core]*1)
                elif template[i][1][0] == "branch_ins":
                    template[i][0] = template[i][0] % int(
                        data[core]['Branch_instructions'])
                elif template[i][1][0] == "branch_mis":
                    template[i][0] = template[i][0] % int(
                        'branch_predictor.num-incorrect' in stats and stats['branch_predictor.num-incorrect'][core] or 0)
                    ...
                    template[i][0] = template[i][0] % int((instrs[core])*0.5)
                # itlb equals icache reads and writes
                elif template[i][1][0] == "itlb.total_accesses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-I.loads'][core] + stats['L1-I.stores'][core])
                elif template[i][1][0] == "itlb.total_misses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-I.load-misses'][core] + stats['L1-I.store-misses'][core])
                elif template[i][1][0] == "icache.read_accesses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-I.loads'][core])
                # dtlb equals dcache reads and writes
                elif template[i][1][0] == "dtlb.total_accesses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-D.loads'][core] + stats['L1-D.stores'][core])
                elif template[i][1][0] == "dtlb.total_misses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-D.load-misses'][core] + stats['L1-D.store-misses'][core])
                elif template[i][1][0] == "dcache.read_accesses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-D.loads'][core])
                elif template[i][1][0] == "dcache.read_misses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-D.load-misses'][core])
                elif template[i][1][0] == "dcache.write_accesses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-D.stores'][core])
                elif template[i][1][0] == "dcache.write_misses":
                    template[i][0] = template[i][0] % int(
                        stats['L1-D.store-misses'][core])
                elif template[i][1][0] == "function_calls":
                    template[i][0] = template[i][0] % int(
                        instrs[core] * 0.35000)
                ...
                # L2 caches
                    ...
                    template[i][0] = template[i][0] % sum(
                        [stats['L2.loads'][c] for c in cores_l2s])
                elif template[i][1][0] == "L2.write_accesses":
                    template[i][0] = template[i][0] % sum(
                        [stats['L2.stores'][c] for c in cores_l2s])
                elif template[i][1][0] == "L2.read_misses":
                    template[i][0] = template[i][0] % sum(
                        [stats['L2.load-misses'][c] for c in cores_l2s])
                elif template[i][1][0] == "L2.write_misses":
                    template[i][0] = template[i][0] % sum(
                        [stats['L2.store-misses'][c] for c in cores_l2s])
                elif template[i][1][0] == "L2_duty_cycle":
                    template[i][0] = template[i][0] % min(1, (sum(
                        [stats['L2.loads'][c] + stats['L2.stores'][c] for c in cores_l2s]) / float(max_system_cycles)))
                # L3 caches
                elif template[i][1][0] == "L3.read_accesses":
                    template[i][0] = template[i][0] % ('L3.loads' in stats and sum(
                        [stats['L3.loads'][c] for c in cores_l3s]) or 0)
                elif template[i][1][0] == "L3.write_accesses":
                    template[i][0] = template[i][0] % ('L3.stores' in stats and sum(
                        [stats['L3.stores'][c] for c in cores_l3s]) or 0)
                elif template[i][1][0] == "L3.read_misses":
                    template[i][0] = template[i][0] % (
                        'L3.load-misses' in stats and sum([stats['L3.load-misses'][c] for c in cores_l3s]) or 0)
                elif template[i][1][0] == "L3.write_misses":
                    template[i][0] = template[i][0] % (
                        'L3.store-misses' in stats and sum([stats['L3.store-misses'][c] for c in cores_l3s]) or 0)
                elif template[i][1][0] == "L3_duty_cycle":
                    template[i][0] = template[i][0] % min(1, ('L3.loads' in stats and sum(
                        [stats['L3.loads'][c] + stats['L3.stores'][c] for c in cores_l3s]) / float(max_system_cycles) or 0))
                else:
                    raise ValueError('Unknown stat template %s' %
                                     template[i][1][0])
...
def readTemplate(ncores, num_l2s, private_l2s, num_l3s, technology_node):
    ...
    template.append(
        ["\t\t<param name=\"number_of_cores\" value=\"%i\"/>" % ncores, ""])
    template.append(
    ...
    template.append(["\t\t<param name=\"Private_L2\" value =\"%i\"/>" %
                     (1 if private_l2s else 0), ""])
    template.append(
        ["\t\t<param name=\"number_of_L3s\" value=\"%i\"/> <!-- This number means how many L3 clusters -->" % num_l3s, ""])
    ...
    template.append(
        ["\t\t<param name=\"core_tech_node\" value=\"%u\"/><!-- nm -->" % technology_node, ""])
    template.append(["\t\t<param name=\"target_core_clockrate\" value='%i'/><!--MHz -->",
                     ["core_clock", "cfg", None]])  # CFG
    template.append(
    ...
    template.append(
        ["\t\t<param name=\"device_type\" value=\"{:d}\"/><!--0: HP(High Performance Type); 1: LSTP(Low standby power) 2: LOP (Low Operating Power)  -->".format(device_type), ""])
    template.append(
...
# --------------------------------------------------------------------------------------------------------------------
    template.append(["\t\t<stat name=\"total_cycles\" value=\"%i\"/>",
                     ["total_system_cycles", "stat", -1]])  # STATS
    template.append(["\t\t<stat name=\"idle_cycles\" value=\"%i\"/>",
                     ["total_system_idle_cycles", "stat", -1]])  # STATS
    template.append(["\t\t<stat name=\"busy_cycles\"  value=\"%i\"/>",
                     ["total_system_busy_cycles", "stat", -1]])  # STATS
# --------------------------------------------------------------------------------------------------------------------
    template.append(
...
#################################################################################################################
    for iCount in range(ncores):
        template.append(["\t\t<component id=\"system.core%i\" name=\"core%i\">" % (
            iCount, iCount), ""])  # check how this can be done/
        template.append(["\t\t<!-- Core property -->", ""])
#       template.append(["\t\t\t<param name=\"clock_rate\" value=\"2660\"/>",""])       #CFG
        template.append(["\t\t\t<param name=\"clock_rate\" value='%i'/>",
                         ["core_clock", "cfg", iCount]])  # CFG
        template.append(
            ["\t\t\t<param name=\"vdd\" value=\"%f\"/><!-- 0 means using ITRS default vdd -->", ["core_vdd", "cfg", iCount]])
        ...
        template.append(
            ["\t\t\t<param name=\"machine_type\" value=\"%i\"/><!-- 1 inorder; 0 OOO-->", ["machineType", "cfg", iCount]])
        ...
        template.append(
            ["\t\t\t<param name=\"fetch_width\" value=\"%d\"/>", ["issue_width", "cfg", iCount]])
        template.append(
            ["\t\t\t<!-- fetch_width determins the size of cachelines of L1 cache block -->", ""])
        template.append(
            ["\t\t\t<param name=\"number_instruction_fetch_ports\" value=\"1\"/>", ""])
        template.append(
            ["\t\t\t<param name=\"decode_width\" value=\"%d\"/>", ["issue_width", "cfg", iCount]])
        template.append(
...
# ---------------------------------------------------------------------------
        template.append(
            ["\t\t\t<param name=\"issue_width\" value=\"%u\"/>", ["issue_width", "cfg", iCount]])
        template.append(["\t\t\t<param name=\"peak_issue_width\" value=\"%u\"/>",
                         ["peak_issue_width", "cfg", iCount]])
        template.append(
        ...
        # assuming the issue and commit widths are equal
        template.append(
            ["\t\t\t<param name=\"commit_width\" value=\"%u\"/>", ["issue_width", "cfg", iCount]])
        template.append(
            ["\t\t\t<!-- commit_width determins the number of ports of register files -->", ""])
        template.append(
            ["\t\t\t<param name=\"fp_issue_width\" value=\"2\"/>", ""])
        # Check whether it can be extracted
        template.append(
        ...
        # "#int_pipelines,#fp_pipelines"; if fp_pipelines=0 they are shared
        template.append(
        ...
        # Check how it can be extracted
        template.append(
            ["\t\t\t<param name=\"pipeline_depth\" value=\"14,14\"/>", ""])
        template.append(
            ["\t\t\t<!-- pipeline depth of int and fp, if pipeline is shared, the second number is the average cycles of fp ops -->", ""])
        template.append(["\t\t\t<!-- issue and exe unit-->", ""])
        template.append(["\t\t\t<param name=\"ALU_per_core\" value=\"%u\"/>",
                         ["ALU_per_core", "cfg", iCount], ""])
        template.append(
        ...
        template.append(
            ["\t\t\t<param name=\"ROB_size\" value=\"%d\"/>", ["window_size", "cfg", iCount]])
        template.append(
...
# Important stats to be extracted
        template.append(["\t\t\t<stat name=\"total_instructions\" value=\"%i\"/>",
                         ["total_instructions", "stat", iCount]])  # stats --
        template.append(["\t\t\t<stat name=\"int_instructions\" value=\"%i\"/>",
                         ["integer_ins", "stat", iCount]])  # stats--
        template.append(["\t\t\t<stat name=\"fp_instructions\" value=\"%i\"/>",
                         ["fp_ins", "stat", iCount]])  # stats--
        template.append(["\t\t\t<stat name=\"branch_instructions\" value=\"%i\"/>",
                         ["branch_ins", "stat", iCount]])  # stats--
        template.append(["\t\t\t<stat name=\"branch_mispredictions\" value=\"%i\"/>",
                         ["branch_mis", "stat", iCount]])  # stats--
        template.append(["\t\t\t<stat name=\"load_instructions\" value=\"%i\"/>",
                         ["loads", "stat", iCount]])  # stats
        template.append(["\t\t\t<stat name=\"store_instructions\" value=\"%i\"/>",
                         ["stores", "stat", iCount]])  # stats
        template.append(["\t\t\t<stat name=\"committed_instructions\" value=\"%i\"/>",
                         ["total_instructions", "stat", iCount]])  # stats
        template.append(["\t\t\t<stat name=\"committed_int_instructions\" value=\"%i\"/>",
                         ["integer_ins", "stat", iCount]])  # stats
        template.append(["\t\t\t<stat name=\"committed_fp_instructions\" value=\"%i\"/>",
                         ["fp_ins", "stat", iCount]])  # stats
        template.append(["\t\t\t<stat name=\"total_cycles\" value=\"%i\"/>",
                         ["total_cycles", "stat", iCount]])  # stats
        # template.append(["\t\t\t<stat name=\"idle_cycles\" value=\"%i\"/>",["","stat",iCount]])  #stats
        template.append(["\t\t\t<stat name=\"idle_cycles\" value=\"%i\"/>",
                         ["idle_cycles", "stat", iCount]])  # stats
        template.append(
            ["\t\t\t<stat name=\"busy_cycles\"  value=\"%i\"/>", ["busy_cycles", "stat", iCount]])
# ----------------------------------------------------------------------------------------------------------------------------
        ...
        template.append(
            ["\t\t\t<stat name=\"ROB_reads\" value=\"%i\"/>", ["ROB_reads", "stat", iCount]])
        template.append(
            ["\t\t\t<stat name=\"ROB_writes\" value=\"%i\"/>", ["ROB_writes", "stat", iCount]])
        template.append(["\t\t\t<!-- RAT accesses -->", ""])
        template.append(["\t\t\t<stat name=\"rename_reads\" value=\"%i\"/>",
                         ["RAT_rename.reads", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"rename_writes\" value=\"%i\"/>",
                         ["RAT_rename.writes", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"fp_rename_reads\" value=\"%i\"/>",
                         ["RAT_fp_rename.reads", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"fp_rename_writes\" value=\"%i\"/>",
                         ["RAT_fp_rename.writes", "stat", iCount]])
        template.append(
            ["\t\t\t<!-- decode and rename stage use this, should be total ic - nop -->", ""])
        template.append(["\t\t\t<!-- Inst window stats -->", ""])
        template.append(["\t\t\t<stat name=\"inst_window_reads\" value=\"%i\"/>",
                         ["instr.reads", "stat", iCount]])  # left to defaults
        template.append(["\t\t\t<stat name=\"inst_window_writes\" value=\"%i\"/>",
                         ["instr.writes", "stat", iCount]])  # left to defaults
        template.append(["\t\t\t<stat name=\"inst_window_wakeup_accesses\" value=\"%i\"/>",
                         ["instr.wakeup", "stat", iCount]])  # left to defaults
        template.append(["\t\t\t<stat name=\"fp_inst_window_reads\" value=\"%i\"/>",
                         ["instr.fp.reads", "stat", iCount]])  # left to defaults
        template.append(["\t\t\t<stat name=\"fp_inst_window_writes\" value=\"%i\"/>",
                         ["instr.fp.writes", "stat", iCount]])  # left to defaults
        template.append(["\t\t\t<stat name=\"fp_inst_window_wakeup_accesses\" value=\"%i\"/>",
                         ["instr.fp.wakeup", "stat", iCount]])  # left to defaults'''
# --------------------------------------------------------------------------------------------------------------------
        template.append(["\t\t\t<!--  RF accesses -->", ""])
        template.append(["\t\t\t<stat name=\"int_regfile_reads\" value=\"%i\"/>",
                         ["RF_accesses.int_regfile_reads", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"float_regfile_reads\" value=\"%i\"/>",
                         ["RF_accesses.fp_regfile_reads", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"int_regfile_writes\" value=\"%i\"/>",
                         ["RF_accesses.int_regfile_writes", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"float_regfile_writes\" value=\"%i\"/>",
                         ["RF_accesses.fp_regfile_writes", "stat", iCount]])
        ...
        template.append(["\t\t\t<stat name=\"function_calls\" value=\"%i\"/>",
                         ["function_calls", "stat", iCount]])  # check regarding the stats
        template.append(
        ...
        template.append(["\t\t\t<stat name=\"ialu_accesses\" value=\"%i\"/>", [
                        "window_switches.ialu_accesses", "stat", iCount]])  # check regarding the stats
        template.append(["\t\t\t<stat name=\"fpu_accesses\" value=\"%i\"/>", [
                        "window_switches.fpu_accesses", "stat", iCount]])  # check regarding the stats
        template.append(["\t\t\t<stat name=\"mul_accesses\" value=\"%i\"/>", [
                        "window_switches.mul_accesses", "stat", iCount]])  # check regarding the stats
        template.append(["\t\t\t<stat name=\"cdb_alu_accesses\" value=\"%i\"/>", [
                        "window_switches.cdb_alu_accesses", "stat", iCount]])  # check regarding the stats
        template.append(["\t\t\t<stat name=\"cdb_mul_accesses\" value=\"%i\"/>", [
                        "window_switches.cdb_mul_accesses", "stat", iCount]])  # check regarding the stats
        template.append(["\t\t\t<stat name=\"cdb_fpu_accesses\" value=\"%i\"/>", [
                        "window_switches.cdb_fpu_accesses", "stat", iCount]])  # check regarding the stats
        template.append(
...
# --------------------------------------------------------------------------------------------------------
        ...
        template.append(["\t\t\t<stat name=\"IFU_duty_cycle\" value=\"%f\"/>",
                         ["IFU.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"LSU_duty_cycle\" value=\"%f\"/>",
                         ["LSU.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"MemManU_I_duty_cycle\" value=\"%f\"/>",
                         ["IFU.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"MemManU_D_duty_cycle\" value=\"%f\"/>",
                         ["MemManU.D.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"ALU_duty_cycle\" value=\"%f\"/>",
                         ["ALU.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"MUL_duty_cycle\" value=\"%f\"/>",
                         ["MUL.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"FPU_duty_cycle\" value=\"%f\"/>",
                         ["FPU.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"ALU_cdb_duty_cycle\" value=\"%f\"/>",
                         ["ALU.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"MUL_cdb_duty_cycle\" value=\"%f\"/>",
                         ["MUL.duty_cycle", "stat", iCount]])
        template.append(["\t\t\t<stat name=\"FPU_cdb_duty_cycle\" value=\"%f\"/>",
                         ["FPU.duty_cycle", "stat", iCount]])
        template.append(
...
# -------------------------------------------------------------------------------------------------------
        ...
        template.append(
            ["\t\t\t<component id=\"system.core%i.predictor\" name=\"PBT\">" % iCount, ""])
        template.append(
...
# -------------------------------------------------------------------------------------------------------------------------
        ...
        template.append(
            ["\t\t\t<component id=\"system.core%i.itlb\" name=\"itlb\">" % iCount, ""])
        # not in graphite (whether correct)
        template.append(
            ["\t\t\t\t<param name=\"number_entries\" value=\"128\"/>", ""])
        template.append(["\t\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",
                         ["itlb.total_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"total_misses\" value=\"%i\"/>",
                         ["itlb.total_misses", "stat", iCount]])
        ...
        template.append(["\t\t\t</component>", ""])
        template.append(
            ["\t\t\t<component id=\"system.core%i.icache\" name=\"icache\">" % iCount, ""])
        template.append(
            ["\t\t\t\t<!-- there is no write requests to itlb although writes happen to it after miss, ", ""])
        template.append(["\t\t\t\twhich is actually a replacement -->", ""])
        template.append(["\t\t\t\t<param name=\"icache_config\" value=\"%i,%i,%i,%i,%i,%i, %i, %i\"/>",
                         ["icache_cfg", "comb", iCount]])  # calculate this properly
...
#       template.append(["\t\t\t\t<param name=\"buffer_sizes\" value=\"%i, 16, %i, %i\"/>",["icache_buffer","comb"]]) #mcpat will crash for some different sizes of 2nd parameter
        # mcpat will crash for some different sizes of 2nd parameter
        template.append(
            ["\t\t\t\t<param name=\"buffer_sizes\" value=\"16, 16, 16, 0\"/>", ""])
        template.append(
            ["\t\t\t\t<!-- cache controller buffer sizes: miss_buffer_size(MSHR),fill_buffer_size,prefetch_buffer_size,wb_buffer_size-->", ""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",
                         ["icache.read_accesses", "stat", iCount]])
        template.append(
            ["\t\t\t\t<stat name=\"read_misses\" value=\"0\"/>", ""])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>", ""])
        template.append(["\t\t\t</component>", ""])
        template.append(
            ["\t\t\t<component id=\"system.core%i.dtlb\" name=\"dtlb\">" % iCount, ""])
        template.append(
            ["\t\t\t\t<param name=\"number_entries\" value=\"256\"/>", ""])
        template.append(["\t\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",
                         ["dtlb.total_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"total_misses\" value=\"%i\"/>",
                         ["dtlb.total_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>", ""])
        template.append(["\t\t\t</component>", ""])
        template.append(
            ["\t\t\t<component id=\"system.core%i.dcache\" name=\"dcache\">" % iCount, ""])
        template.append(
            ["\t\t\t\t<!-- all the buffer related are optional -->", ""])
        template.append(
            ["\t\t\t\t<param name=\"dcache_config\" value=\"%i,%i,%i,%i,%i,%i, %i, %i\"/>", ["dcache_cfg", "comb", iCount]])
        ...
        template.append(
            ["\t\t\t\t<!-- cache controller buffer sizes: miss_buffer_size(MSHR),fill_buffer_size,prefetch_buffer_size,wb_buffer_size-->", ""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",
                         ["dcache.read_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_accesses\" value=\"%i\"/>",
                         ["dcache.write_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"read_misses\" value=\"%i\"/>",
                         ["dcache.read_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_misses\" value=\"%i\"/>",
                         ["dcache.write_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>", ""])
        template.append(["\t\t\t</component>", ""])
        template.append(
            ["\t\t\t<component id=\"system.core%i.BTB\" name=\"BTB\">" % iCount, ""])
        template.append(
            ["\t\t\t\t<!-- all the buffer related are optional -->", ""])
        # ( (64 target + 3 type + 4 ffset + 3 PLRU bits) * 512 entries * 4 ways ) / 8 bits-per-byte = 18944 bytes; tag overheads are already taken into account; block size = 8, associativity = 4, num-of-banks = 1, throughput = 1, latency = 3
        template.append(
            ["\t\t\t\t<param name=\"BTB_config\" value=\"18944,8,4,1, 1,3\"/>", ""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",
                         ["BTB.read_accesses", "stat", iCount]])
        template.append(
    ...
    if 0:  # not currently modeling any L1 directory
        template.append(
            ["\t\t<component id=\"system.L1Directory%i\" name=\"L1Directory%i\">" % (iCount, iCount), ""])
        template.append(
        ...
        template.append(
            ["\t\t\t\t<param name=\"clockrate\" value=\"%i\"/>", ["core_clock", "cfg", iCount]])
        template.append(
            ["\t\t\t\t<param name=\"ports\" value=\"1,1,1\"/>", ""])
        template.append(
            ["\t\t\t\t<param name=\"device_type\" value=\"{:d}\"/>".format(device_type), ""])
        ...
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",
                         ["L1_directory.read_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_accesses\" value=\"%i\"/>",
                         ["L1_directory.write_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"read_misses\" value=\"%i\"/>",
                         ["L1_directory.read_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_misses\" value=\"%i\"/>",
                         ["L1_directory.write_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"%i\"/>",
                         ["L1_directory.conflicts", "stat", iCount]])
        template.append(["\t\t</component>", ""])
# ------------------------------------------------------------------------------------------------------------------------
    if 0:  # not currently modeling any L2 directory
        template.append(
            ["\t\t<component id=\"system.L2Directory%i\" name=\"L2Directory%i\">" % (iCount, iCount), ""])
        template.append(
        ...
        template.append(["\t\t\t\t<!-- altough there are multiple access types, Performance simulator needs to cast them into reads or writes         e.g. the invalidates can be considered as writes -->", ""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",
                         ["L2_directory.read_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_accesses\" value=\"%i\"/>",
                         ["L2_directory.write_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"read_misses\" value=\"%i\"/>",
                         ["L2_directory.read_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_misses\" value=\"%i\"/>",
                         ["L2_directory.write_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"%i\"/>",
                         ["L2_directory.conflicts", "stat", iCount]])
        template.append(["\t\t</component>", ""])
# ------------------------------------------------------------------------------------------------------------------------
    for iCount in range(num_l2s):
        template.append(
            ["\t\t<component id=\"system.L2%i\" name=\"L2%i\">" % (iCount, iCount), ""])
        template.append(
            ["\t\t\t\t<param name=\"L2_config\" value=\"%i,%i,%i,%i,%i,%i, %i, %i\"/>", ["L2_config", "comb", iCount]])
        #template.append(["\t\t\t\t<param name=\"L2_config\" value=\"6291456,64, 16, 8, 8, 23, 32, 1\"/>",""])
        template.append(
            ["\t\t\t\t<param name=\"buffer_sizes\" value=\"16, 16, 16, 16\"/>", ""])
        template.append(
            ["\t\t\t\t<param name=\"clockrate\" value=\"%i\"/>", ["L2_clock", "cfg", iCount]])
        template.append(
            ["\t\t\t\t<param name=\"vdd\" value=\"%f\"/><!-- 0 means using ITRS default vdd -->", ["L2_vdd", "cfg", iCount]])
        template.append(
            ["\t\t\t\t<param name=\"ports\" value=\"1,1,1\"/>", ""])
        template.append(
            ["\t\t\t\t<param name=\"device_type\" value=\"{:d}\"/>".format(device_type), ""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",
                         ["L2.read_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_accesses\" value=\"%i\"/>",
                         ["L2.write_accesses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"read_misses\" value=\"%i\"/>",
                         ["L2.read_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"write_misses\" value=\"%i\"/>",
                         ["L2.write_misses", "stat", iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>", ""])
        template.append(
            ["\t\t\t\t<stat name=\"duty_cycle\" value=\"%f\"/>", ["L2_duty_cycle", "stat", iCount]])
        template.append(["\t\t</component>", ""])
# ------------------------------------------------------------------------------------------------------------------------
    for iCount in range(num_l3s):
        template.append(
            ["\t\t<component id=\"system.L3%i\" name=\"L3%i\">" % (iCount, iCount), ""])
        template.append(
            ["\t\t\t\t<param name=\"L3_config\" value=\"%i,%i,%i, %i, %i, %i,%i\"/>", ["L3_config", "comb", iCount]])
        #template.append(["\t\t\t<param name=\"L3_config\" value=\"16777216 , 64 ,16, 16, 16, 100,1\"/>",""])
        template.append(
            ["\t\t<!-- the parameters are capacity,block_width, associativity,bank, throughput w.r.t. core clock, latency w.r.t. core clock,-->", ""])
        template.append(
            ["\t\t\t\t<param name=\"clockrate\" value=\"%i\"/>", ["L3_clock", "cfg", iCount]])
        template.append(
            ["\t\t\t\t<param name=\"vdd\" value=\"%f\"/><!-- 0 means using ITRS default vdd -->", ["L3_vdd", "cfg", iCount]])
        template.append(
            ["\t\t\t\t<param name=\"ports\" value=\"1,1,1\"/>", ""])
        template.append(
            ["\t\t\t\t<param name=\"device_type\" value=\"{:d}\"/>".format(device_type), ""])
        ...
        template.append(
            ["\t\t\t\t<!-- cache controller buffer sizes: miss_buffer_size(MSHR),fill_buffer_size,prefetch_buffer_size,wb_buffer_size-->", ""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",
                         ["L3.read_accesses", "stat", iCount]])  # populate with stats file
        template.append(["\t\t\t\t<stat name=\"write_accesses\" value=\"%i\"/>",
                         ["L3.write_accesses", "stat", iCount]])  # populate with stats file
        template.append(["\t\t\t\t<stat name=\"read_misses\" value=\"%i\"/>",
                         ["L3.read_misses", "stat", iCount]])  # populate with stats file
        template.append(["\t\t\t\t<stat name=\"write_misses\" value=\"%i\"/>",
                         ["L3.write_misses", "stat", iCount]])  # populate with stats file
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>", ""])
        template.append(
            ["\t\t\t\t<stat name=\"duty_cycle\" value=\"%f\"/>", ["L3_duty_cycle", "stat", iCount]])
        template.append(["\t\t</component>", ""])
# ------------------------------------------------------------------------------------------------------------------------
# END FOR LOOP
...
#template.append(["\t\t<component id=\"system.NoC%i\" name=\"noc%i\">"%(iCount,iCount),""])
    ...
    template.append(
        ["\t\t\t<param name=\"clockrate\" value=\"%i\"/>", ["NoC_clock", "cfg", iCount]])
    template.append(
        ["\t\t\t<param name=\"vdd\" value=\"%f\"/><!-- 0 means using ITRS default vdd -->", ["NoC_vdd", "cfg", iCount]])
# template.append(["\t\t\t<param name=\"clock_rate\" value='%i'/>",["clockFrequency","cfg"]])   #CFG
    template.append(
        ["\t\t\t<param name=\"type\" value=\"%d\"/>", ["NoC.type", "stat", -1]])
    template.append(
    ...
    template.append(["\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",
                     ["NoC.total_accesses", "stat", -1]])  # populate with stats file
    template.append(["\t\t\t<stat name=\"duty_cycle\" value=\"%f\"/>",
                     ["NoC.duty_cycle", "stat", -1]])  # check whether the value is right
    template.append(["\t\t</component>", ""])
###################################################################################################################
# ------------------------------------------------------------------------------------------------------------------------
    template.append(
  ...
  template.append(["\t\t\t</component>",""])
  '''
    template.append(["\t\t<component id=\"system.mc\" name=\"mc\">", ""]
                    )  # It is required to add this component, however we set number_mcs=0
    template.append(
    ...
    template.append(["\t\t\t<stat name=\"memory_accesses\" value=\"%i\"/>",
                     ["memory.accesses", "stat", -1]])
    template.append(
        ["\t\t\t<stat name=\"memory_reads\" value=\"%i\"/>", ["memory.reads", "stat", -1]])
    template.append(
        ["\t\t\t<stat name=\"memory_writes\" value=\"%i\"/>", ["memory.writes", "stat", -1]])
    template.append(
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hj:t:c:d:o:", [
                                   'no-graph', 'no-text', 'partial='])
    ...
    main(jobid=jobid, resultsdir=resultsdir, powertype=powertype, config=config,
         outputfile=outputfile, no_graph=no_graph, print_stack=not no_text, partial=partial)
